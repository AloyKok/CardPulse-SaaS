...
begin
  ...
  loop
    ...
    if v_kind = 'misc' then
      ...
      v_unit_cost := round((v_unit_price * 0.20)::numeric, 2);
      ...
      v_lines := v_lines || jsonb_build_array(jsonb_build_object(
        ...
        'lineProfit', (v_unit_price - v_unit_cost) * v_qty,
        'costUnknown', false
      ));
  ...
  values (
    ...
    v_cost_total, v_total - v_cost_total, false,
...
with rebuilt as (
  ...
  cross join lateral (
    select case
      when coalesce(nullif(lines.line ->> 'costUnknown', '')::boolean, false)
